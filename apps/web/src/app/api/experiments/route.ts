...
export async function POST(request: Request) {
  ...
  if (!(await withinRateLimit(request, "experiments", 1))) return rateLimitResponse();
  ...
  if (!(await queueHasCapacity(parsed.data.runCount))) {
    return Response.json({ code: "QUEUE_CAPACITY_EXCEEDED" }, { status: 429, headers: { "retry-after": "30" } });
  }
  try {
    const experiment = await createExperiment(parsed.data.sourceEvaluationId, parsed.data.name, parsed.data.runCount);
    return Response.json({ experimentId: experiment.id }, { status: 201 });
  } catch {
    return Response.json({ code: "EXPERIMENT_CREATE_FAILED" }, { status: 409 });
  }
}
