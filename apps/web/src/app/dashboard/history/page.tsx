  ...
  return (
    <main className="mx-auto w-full max-w-[1200px] px-4 py-8 md:px-7 md:py-10">
      ...
      <nav aria-label={text({ en: "Filter evaluation type", ja: "評価タイプで絞り込む", zh: "按评价类型筛选" })} className="mt-6 flex flex-wrap gap-2">
        {(["all", "website", "logo"] as const).map((value) => <Link className={`rounded-[var(--radius-control)] border px-3 py-2 text-xs ${selected === value ? "border-[var(--accent)]/50 bg-[rgba(255,255,255,0.07)] text-[var(--text)]" : "border-white/10 text-[var(--text-secondary)]"}`} href={value === "all" ? "/dashboard/history" : `/dashboard/history?type=${value}`} key={value}>{value === "all" ? text({ en: "All", ja: "すべて", zh: "全部" }) : value === "website" ? text({ en: "Websites", ja: "Webサイト", zh: "网页" }) : "Logo"}</Link>)}
      </nav>
      ...
      {unavailable ? (
        ...
        <section className="workspace-section mt-7 grid min-h-72 place-items-center border-y border-white/[0.08] py-12 text-center"><div><Database aria-hidden="true" className="mx-auto size-7 text-[var(--accent)]" /><h2 className="mt-5 text-xl font-medium">{text({ en: "No evaluation history", ja: "評価履歴がありません", zh: "还没有评估历史" })}</h2><p className="mt-2 text-sm text-[var(--text-secondary)]">{text({ en: "Create an evaluation to start this history.", ja: "最初の評価を作成すると、ここに結果が表示されます。", zh: "创建第一次评估后，结果会显示在这里。" })}</p><div className="mt-6 flex flex-wrap justify-center gap-3"><ButtonLink href="/dashboard/evaluations/new">{text({ en: "Evaluate website", ja: "Webサイトを評価", zh: "评价网页" })}</ButtonLink><ButtonLink href="/dashboard/logo" tone="secondary">{text({ en: "Evaluate logo", ja: "ロゴを評価", zh: "评价 Logo" })}</ButtonLink></div></div></section>
      ) : (
