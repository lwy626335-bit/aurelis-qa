...
export default async function NewEvaluationPage() {
  ...
  return (
    <main data-layout="form" className="mx-auto w-full max-w-4xl px-4 py-8 md:px-7 md:py-12">
      <header>
      <h1 className="text-4xl font-medium tracking-[-0.045em] md:text-5xl">{copy.createTitle}</h1>
      <p className="mt-4 max-w-2xl text-sm leading-7 text-[var(--text-secondary)]">{copy.createDescription}</p>

      <div className="mt-7 flex items-start gap-3 rounded-[var(--radius-control)] border border-[rgba(255,255,255,0.17)] bg-[rgba(255,255,255,0.045)] p-4 text-xs leading-5 text-[var(--text-secondary)]">
        <Info aria-hidden="true" className="mt-0.5 size-4 shrink-0 text-[var(--accent)]" />
        <p>{copy.workerNotice}</p>
      </div>
