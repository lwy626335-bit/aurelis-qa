...
const navigation = [
  { label: "Overview", href: "/dashboard", icon: Gauge, phase: 1 },
  { label: "Evaluations", href: "/dashboard/evaluations", icon: ChartLineUp, phase: 2 },
  { label: "Brand profiles", href: "/dashboard/brands", icon: Sparkle, phase: 2 },
  { label: "History", href: "/dashboard/history", icon: ClockCounterClockwise, phase: 6 },
  { label: "Rubrics", href: "/dashboard/rubrics", icon: SlidersHorizontal, phase: 6 },
  { label: "Research", href: "/dashboard/research", icon: Flask, phase: 6 },
];
  ...
  return (
    <>
      <div className="flex h-16 items-center border-b border-white/[0.07] px-5">
        <Link href="/" aria-label="AURELIS home" className="text-[var(--text)] hover:text-[var(--accent)]" onClick={close}>
          <AurelisMark />
        </Link>
      </div>
      <nav className="flex-1 px-3 py-5" aria-label="Dashboard navigation">
        <p className="px-3 pb-3 font-mono text-[9px] tracking-[0.12em] text-[var(--text-tertiary)] uppercase">Workspace</p>
      ...
      <div className="border-t border-white/[0.07] p-4">
        ...
        >
          <BookOpenText aria-hidden="true" className="size-4" /> Documentation
        </Link>
        <div className="mt-3 rounded-[var(--radius-control)] border border-white/[0.07] bg-white/[0.025] p-3">
          <div className="flex items-center gap-2 text-[10px] text-[var(--text-secondary)]">
            <BezierCurve aria-hidden="true" className="size-3.5 text-[var(--accent)]" /> Phase 1
          </div>
      ...
      <div className="lg:col-start-2">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-white/[0.07] bg-[rgba(7,8,11,0.88)] px-4 backdrop-blur-xl lg:px-7">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="grid size-8 place-items-center rounded-full border border-white/10 bg-white/[0.04] text-[10px] font-semibold text-[var(--text-secondary)]" aria-label="Demo workspace">
              NA
            </div>
          </div>
        </header>
