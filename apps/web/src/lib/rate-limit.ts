...
function clientKey(request: Request, scope: string) {
  const identity = request.headers.get("x-forwarded-for")?.split(",").at(-1)?.trim()
    || request.headers.get("authorization")
    || "anonymous";
  return `${scope}:${createHash("sha256").update(identity).digest("hex")}`;
}

export async function withinRateLimit(request: Request, scope: string, limit: number, windowMs = 60_000) {
...
export function rateLimitResponse() {
  return Response.json({ code: "RATE_LIMITED" }, { status: 429, headers: { "retry-after": "60" } });
}
